...
int main(int argc, const char* argv[]) {
    ...
    while (EAGAIN != errno && EWOULDBLOCK != errno){
        ...
        if (ret2 == -1){
            ...
            uint8_t* unsignbuff = (uint8_t*)buffer;

            uint8_t group_id = unsignbuff[0];
            
            // rebuilding the message by shifting the bytes back 
            uint32_t message =  (unsignbuff[1] << 24) |
                                (unsignbuff[2] << 16) |
                                (unsignbuff[3] << 8) | 
                                 unsignbuff[4];

            uint32_t signature = secret ^ message;

            char cinco_byte[5];

            cinco_byte[0] = group_id;

            cinco_byte[1] = (signature >> 24) & 0xFF;
            cinco_byte[2] = (signature >> 16) & 0xFF;
            cinco_byte[3] = (signature >> 8) & 0xFF;
            cinco_byte[4] = signature & 0xFF;

            ret = sendto(sockfd, cinco_byte, 5, 0, (struct sockaddr*)&srcaddr, sizeof(srcaddr));
            
            cout << "Group ID " << group_id << " Challenge_num " << message << endl;

            char fina[256];
            socklen_t finalen =sizeof(srcaddr);

            while(true){    
                int final_ret = recvfrom(sockfd, fina, sizeof(fina), 0, (struct sockaddr*)&srcaddr, &finalen);
                ...
                if (final_ret > 5){  
                    s = string(fina, final_ret); //creating a value for the received respond
                ...
                char msgport4013[4];
                msgport4013[0] = (signature >> 24) & 0xFF;
                msgport4013[1] = (signature >> 16) & 0xFF;
                msgport4013[2] = (signature >> 8) & 0xFF;
                msgport4013[3] = signature & 0xFF;
                
                struct sockaddr_in addr4013 = destaddr;
                addr4013.sin_port = htons(4013);

                int ret4013 = sendto(sockfd, msgport4013, 4, 0, (struct sockaddr*)&addr4013, sizeof(addr4013));
                if (ret4013 < 0){
                    cout << "bruh aint no way" << endl;
                }

                else{
                    char buffer4013[1024];
                    struct sockaddr_in reply_addr;
                    socklen_t reply_len = sizeof(reply_addr);
                        
                    int ret_recv4013 = recvfrom(sockfd, buffer4013, sizeof(buffer4013), 0, (struct sockaddr*)&reply_addr, &reply_len);
                    if (ret_recv4013 < 0){
                        cout << "bruh aint no way" << endl;
                    }

                    else{
                        string s4013(buffer4013, ret_recv4013);
                        int send_port = ntohs(reply_addr.sin_port);
                        cout << "port " << send_port << " replied " << s4013 << endl;

                        int len = ret_recv4013;
                        uint16_t target_checksum = ((uint8_t)buffer4013[len - 6] << 8) | (uint8_t)buffer4013[len - 5];
                        uint32_t target_source_ip = ((uint8_t)buffer4013[len - 4] << 24) |
                                                    ((uint8_t)buffer4013[len - 3] << 16) |
                                                    ((uint8_t)buffer4013[len - 2] << 8)  |
                                                    (uint8_t)buffer4013[len - 1];
