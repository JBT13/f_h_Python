...
class DatabaseAPI:
    ...
    def add_competition(self, username:str, place: str, held: date | None=None) -> int:
        """
    ...
    def retrieve_results_from_sports_and_places_page(self, places: list[str], sports: list[str],
        ...
        try:
            ...
            query = """
                SELECT C.place, C.held, S.name as Sport, A.ID as AthleteID, A.name as name, R.result
                FROM Sports S
                JOIN Results R ON R.sportID = S.ID
                JOIN Athletes A ON A.ID = R.athleteID
                JOIN Competitions C ON C.ID = R.competitionID
            """

            if len(places) != 0:
                query += f"WHERE C.place IN ('{"','".join(places)}')"

            if len(places) != 0 and len(sports) != 0:
                query += f" AND S.name IN ('{"','".join(sports)}')"

            elif len(sports) != 0:
                query += f"WHERE S.name IN ('{"','".join(sports)}')"
            ...
            if sort_by is not None:
                ...
                if key not in columns:
                    raise pg.errors.DataError(f'The provided sort key does not match any columns of the Competitions table! Key: {key}')
                if sort_by['order'] == 'desc':
                    rows = self.connection.execute(f"{query} ORDER BY {key} DESC").fetchall()
                else:
                    rows = self.connection.execute(f"{query} ORDER BY {key}").fetchall()
            else:
