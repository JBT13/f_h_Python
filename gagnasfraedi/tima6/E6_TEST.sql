...
-- Copyright (C) 2016-2021, Bjorn Thor Jonsson

-- ---------------------------------------------------------------------
select '1. View AllRecords' as now_testing;
...
select 'This should not give error as updates to bIsPaid are allowed' as result;
...
-- ---------------------------------------------------------------------
select '3. Trigger on Records' as now_testing;
...
select 'There should be 1 bogus account' as result;
...
insert into Records ( AID, rType, rAmount )
values ( 5, 'T', -100000 );
...
select 'BONUS: This should still return 1' as result;

select count(*) as bogus
from FindBogusAccounts();

select 'Just in case we do not get an empty result, then check it was account 5 (in addition to account 90)' as result;

select *
from FindBogusAccounts() A left outer join Records R on A.AID = R.AID;
...
select * 
from Records
where RID >= -1 + (select max(RID) from Records);
...
select *
from Records 
where RID = (select max(RID) from Records);
