...
ERROR:  column "aid" does not exist
LINE 1: select PID, AID, sum(rAmount), max(aBalance), case sum(rAmou...
                    ^
HINT:  Perhaps you meant to reference the column "allrecords.pid" or the column "allrecords.rid".
                               result                               
--------------------------------------------------------------------
...
CONTEXT:  SQL expression "NEW.ID"
PL/pgSQL function checkbills1() line 9 at RAISE
ROLLBACK
BEGIN
                     result                      
-------------------------------------------------
...
ERROR:  You are in the past
CONTEXT:  PL/pgSQL function checkbills1() line 13 at RAISE
ROLLBACK
BEGIN
                            result                            
--------------------------------------------------------------
 This should not give error as updates to bIsPaid are allowed
(1 row)
...
CREATE FUNCTION
             result              
---------------------------------
 There should be 1 bogus account
(1 row)
...
ERROR:  null value in column "rdate" of relation "records" violates not-null constraint
DETAIL:  Failing row contains (483, 5, null, T, -100000, null).
ROLLBACK
             result              
---------------------------------
 There should be 1 bogus account
...
ERROR:  null value in column "rdate" of relation "records" violates not-null constraint
DETAIL:  Failing row contains (484, 5, null, T, 100000, null).
