...
class MyMap(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        index = self._in_array(self._array[bucket_id], key)
    ...
    def __getitem__(self, key):
        ...
        """
        bucket = h(key,len(self._array))
        index = self._in_array(self._array[bucket], key)
        if index is not None:
            return self._array[bucket][index].value
    ...
    def __delitem__(self, key):
        ...
        """
        bucket = h(key, len(self._array))
        entry = self._in_array(self._array[bucket], key)
        if entry is not None:
            item = self._array[bucket][entry]
            del self._array[bucket][entry]
            self._len -= 1
            return item.value
    ...
    def _resize(self):
        resize_array = [[] for _ in range(len(self._array)*2)]
        for i in self:
            bucket_id = h(i.key, len(resize_array))
            resize_array[bucket_id].append(i)
        self._array = resize_array
